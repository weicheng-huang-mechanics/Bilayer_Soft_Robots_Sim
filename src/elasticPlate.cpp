...
{
	// define material and geometry params
	YoungM = m_YoungM;
	density = m_density;
	radius = m_radius;
	Possion = m_Possion;
	...
	EA = YoungM * radius;
	EI = YoungM * radius * radius * radius / 4;
	GJ = YoungM/(2.0*(1.0+Possion)) * radius * radius * radius / 2;
...
void elasticPlate::updateTimeStep()
{
	...
	for (int i = 0; i < edgeNum; i++)
	{
		v_edgeElement[i].t1_old = v_edgeElement[i].t1;
		v_edgeElement[i].d1_old = v_edgeElement[i].d1;
		v_edgeElement[i].d2_old = v_edgeElement[i].d2;
	}
...
void elasticPlate::computeEdge()
{
	...
	for (int i = 0; i < edge.size(); i++)
	{
		...
		m_edgeElement.m1 = m_edgeElement.d1;
		m_edgeElement.m2 = m_edgeElement.d2;
...
void elasticPlate::updateEdgePair()
{
	for (int i = 0; i < edgeNum; i++)
	{
		...
		v_edgeElement[i].x_1_old = getVertexOld(v_edgeElement[i].nv_1);
		v_edgeElement[i].x_2_old = getVertexOld(v_edgeElement[i].nv_2);

		Vector3d x_1_old = getVertexOld(v_edgeElement[i].nv_1);
		Vector3d x_2_old = getVertexOld(v_edgeElement[i].nv_2);
		Vector3d x_1 = getVertex(v_edgeElement[i].nv_1);
		Vector3d x_2 = getVertex(v_edgeElement[i].nv_2);

		Vector3d d1, d2;
		Vector3d d1_old = v_edgeElement[i].d1_old;
		Vector3d d2_old = v_edgeElement[i].d2_old;
		// Vector3d t1_old = v_edgeElement[i].t1_old;

		Vector3d t1_old = (v_edgeElement[i].x_2_old - v_edgeElement[i].x_1_old) / (v_edgeElement[i].x_1_old - v_edgeElement[i].x_2_old).norm();

		Vector3d t1 = (v_edgeElement[i].x_2 - v_edgeElement[i].x_1) / (v_edgeElement[i].x_1 - v_edgeElement[i].x_2).norm();

		parallelTansport(d1_old, t1_old, t1, d1);
		d2 = t1.cross(d1);
		v_edgeElement[i].d1 = d1;
		v_edgeElement[i].d2 = d2;
		v_edgeElement[i].t1 = t1;

		v_edgeElement[i].theta = getTheta(i);
		double cs, ss;

        cs = cos(v_edgeElement[i].theta);
        ss = sin(v_edgeElement[i].theta);
        v_edgeElement[i].m1 =  cs * v_edgeElement[i].d1 + ss * v_edgeElement[i].d2;
        v_edgeElement[i].m2 = -ss * v_edgeElement[i].d1 + cs * v_edgeElement[i].d2;
	}
}
